...
The **poppadom** is an example item. You must have an bindable function named `SCOItem` with the BoolValue `AgeRestricted` and NumberValue `Cost` outside the handle. You can also have the BoolValue `EAS_Active` inside the handle if you own the anti-theft gates.
